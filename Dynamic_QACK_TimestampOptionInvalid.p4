...
header payload_t {
    ...
    bit<8>          qid;
    bit<8>          sep4;
    bit<32>         counter_flow1;
    bit<8>          sep5;
    bit<32>         counter_flow2;
    bit<8>          sep6;
    varbit<12112>   data;
}
...
control MyIngress(inout headers hdr,
    ...
    apply {
        if(hdr.ipv4.isValid()){
            ...
            //パケットカウント処理
            PacketCount_t tmp = 0;
            PacketCount_t tmp1 = 0;
            PacketCount_t counter_all = 0;
            PacketCount_t counter_flow1 = 0;
            PacketCount_t counter_flow2 = 0;
            PacketCount_t counter_flow3 = 0;

            /*
            bit<32> index_hash = (bit<32>)0;
            hash(index_hash, HashAlgorithm.crc16, (bit<32>)0, {  hdr.ipv4.srcAddr, 
                                                            hdr.ipv4.dstAddr }, NUM_REGISTER);
            bit<32> index_proto = (bit<32>)hdr.ipv4.protocol;
            */

            bit<32> index_all = (bit<32>)0;
            bit<32> index_flow1 = (bit<32>)1;
            bit<32> index_flow2 = (bit<32>)2;
            bit<32> index_flow3 = (bit<32>)3;
            @atomic{
                //tmpに値が書き込まれるのはDATAパケットのときだけ
                if(hdr.ipv4.protocol == 6 && hdr.ipv4.srcAddr == 0x0a000101){
                    pkt_counter.read(tmp, index_all);
                    tmp = tmp + 1;
                    pkt_counter.write(index_all, tmp);
                }

                if(hdr.ipv4.dstAddr == 0x0a000202){
                    pkt_counter.read(tmp1, index_flow1);
                    tmp1 = tmp1 + 1;
                    //hdr.payload.counter_flow1 = tmp1;
                    pkt_counter.write(index_flow1, tmp1);
                }

                if(hdr.ipv4.dstAddr == 0x0a000505){
                    pkt_counter.read(tmp1, index_flow2);
                    tmp1 = tmp1 + 1;
                    //hdr.payload.counter_flow2 = tmp1;
                    pkt_counter.write(index_flow2, tmp1);
                }

                if(hdr.ipv4.dstAddr == 0x0a000606){
                    pkt_counter.read(tmp1, index_flow3);
                    tmp1 = tmp1 + 1;
                    //hdr.payload.counter_flow3 = tmp1;
                    pkt_counter.write(index_flow3, tmp1);
                }

                //counter_*には、どのパケットのときでも値が書き込まれる。
                pkt_counter.read(counter_all, index_all);
                pkt_counter.read(counter_flow1, index_flow1);
                pkt_counter.read(counter_flow2, index_flow2);
                pkt_counter.read(counter_flow3, index_flow3);
            }
            ...
            //次のif文が発動するのをh1からのDATAパケットが来たときにするためにtmpを条件文に含ませる
            if(tmp > 250000 && (bit<11>)tmp == 0){
                PacketCount_t max = counter_flow1;
                if(max < counter_flow2){
                    max = counter_flow2;
                }else if(max < counter_flow3){
                    max = counter_flow3;
                }
                if(max == counter_flow1 && hdr.ipv4.totalLen > 1024 && hdr.ipv4.dstAddr == 0x0a000202){
                    clone_preserving_field_list(CloneType.I2E, RETURN_CLONE_SESSION_ID, 0);
                    counter_flow1 = 0;
                    counter_flow2 = 0;
                    counter_flow3 = 0;
                    @atomic{
                        pkt_counter.write(index_flow1, counter_flow1);
                        pkt_counter.write(index_flow2, counter_flow2);
                        pkt_counter.write(index_flow3, counter_flow3);
                    }
                }

                if(max == counter_flow2 && hdr.ipv4.totalLen > 1024 && hdr.ipv4.dstAddr == 0x0a000505){
                    clone_preserving_field_list(CloneType.I2E, RETURN_CLONE_SESSION_ID, 0);
                    counter_flow1 = 0;
                    counter_flow2 = 0;
                    counter_flow3 = 0;
                ...
                if(max == counter_flow3 && hdr.ipv4.totalLen > 1024 && hdr.ipv4.dstAddr == 0x0a000606){
                    clone_preserving_field_list(CloneType.I2E, RETURN_CLONE_SESSION_ID, 0);
                    counter_flow1 = 0;
                    counter_flow2 = 0;
                    counter_flow3 = 0;
    ...
    apply { 
        if(hdr.ipv4.isValid()){
            ...
            if(IS_I2E_CLONE(standard_metadata)){
                ...
                if(hdr.ipv4.srcAddr == 0x0a000505){
                    hdr.ethernet.dstAddr = hdr.ethernet.srcAddr;
                    hdr.ethernet.srcAddr = 0x080000000500;
                }
                if(hdr.ipv4.srcAddr == 0x0a000606){
                    hdr.ethernet.dstAddr = hdr.ethernet.srcAddr;
            ...
            /*
            //1024byte以上のペイロードを持つパケットに対して、キュー長などを書き込んでwiresharkで確認する
            if(hdr.payload.isValid()){
                if(hdr.ipv4.totalLen > 1024){
                    hdr.payload.deq_timedelta = standard_metadata.deq_timedelta;
                    hdr.payload.sep1 = 255;
                    hdr.payload.enq_qdepth = (bit<24>)standard_metadata.enq_qdepth;
                    hdr.payload.sep2 = 255;
                    hdr.payload.deq_qdepth = (bit<24>)standard_metadata.deq_qdepth;
                    hdr.payload.sep3 = 255;
                    hdr.payload.qid = (bit<8>)standard_metadata.qid;
                    hdr.payload.sep4 = 255;
                    hdr.payload.sep5 = 255;
                    hdr.payload.sep6 = 255;
                    hdr.tcp.checksum = 0;
                }
            }
            */
     ...
     apply {
        ...
        update_checksum(    
            ...
            { hdr.ipv4.srcAddr,          // Pseudo-header: Source IP Address
              ...
              hdr.payload.qid,
              hdr.payload.sep4,
              hdr.payload.counter_flow1,
              hdr.payload.sep5,
              hdr.payload.counter_flow2,
              hdr.payload.sep6,
              hdr.payload.data
            },         
            hdr.tcp.checksum,
